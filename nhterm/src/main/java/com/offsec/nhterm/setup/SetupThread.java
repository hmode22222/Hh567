...
private void copyProotScript() {
    try {
        File destDir = new File(NeoTermPath.USR_PATH + "/bin");
        if (!destDir.exists()) {
            destDir.mkdirs();
        }
        
        InputStream in = context.getAssets().open("kali-proot.sh");
        FileOutputStream out = new FileOutputStream(new File(destDir, "kali-proot"));
        ...
        File script = new File(destDir, "kali-proot");
